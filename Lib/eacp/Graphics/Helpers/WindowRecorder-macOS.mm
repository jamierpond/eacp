...
// Set by the recorder; WindowRecorder::Native is private, so frames are
    ...
    API_AVAILABLE(macos(12.3))
{
    ...
    height -= height & 1;
    if (width <= 0 || height <= 0)
        return fail(error, "WindowRecorder: window has no size");
...
Image cgImageToImage(CGImageRef cgImage, std::string* error)
{
    ...
    if (!destination)
    {
        setError(error, "WindowRecorder: could not create a PNG encoder");
        return {};
    }
    ...
    if (!encoded)
    {
        setError(error, "WindowRecorder: PNG encode failed");
        return {};
    }
...
Image captureWindowImage(void* nativeWindow, std::string* error)
{
    ...
    if (!nsWindow)
    {
        setError(error, "WindowRecorder: no native window to capture");
        return {};
    }
    ...
    if (@available(macOS 14, *))
    {
        ...
        {
            setError(error,
                     shotError ? std::string {"WindowRecorder: "}
                                     + shotError.localizedDescription.UTF8String
                               : std::string {"WindowRecorder: capture "
                                              "returned no image"});
            if (shot)
                CFRelease(shot);
            return {};
    ...
    setError(error, "WindowRecorder: window image capture requires macOS 14+");
    return {};
}
} // namespace
...
bool WindowRecorder::start(Window& window,
...
bool WindowRecorder::start(View& view,
                           const std::string& path,
                           Options options,
                           std::string* error)
{
...
bool WindowRecorder::startWindow(void* nativeWindow,
                                 const std::string& path,
                                 Options options,
                                 std::string* error)
{
    if (impl->recording)
        return fail(error, "WindowRecorder: already recording");

    auto* nsWindow = (__bridge NSWindow*) nativeWindow;
    if (!nsWindow)
        return fail(error, "WindowRecorder: no native window to record");
    ...
    if (@available(macOS 12.3, *))
    {
        ...
        if (![impl->writer startWriting])
        {
            ...
            impl->input = nil;
            impl->adaptor = nil;
            return fail(error, "WindowRecorder: failed to start writing");
        }
        ...
        impl->queue =
            dispatch_queue_create("ai.eacp.windowrecorder", DISPATCH_QUEUE_SERIAL);
    ...
    return fail(error, "WindowRecorder: window recording requires macOS 12.3+");
}
