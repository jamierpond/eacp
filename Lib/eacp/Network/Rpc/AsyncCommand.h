...
// becomes a typed one. Lived in the WebView bridge first; now the IPC
// bridge speaks it too, so it sits where both can reach it.
namespace eacp::Rpc
{
...
// Selects how a bridge executes a command when turning a caller's request
// into an async one. The C++ handler is always an ordinary synchronous
// function - this only chooses where/when the bridge runs it. The caller
// sees an async completion either way (it always has).
enum class CommandExecution
{
    ...
    // common case for UI/state handlers); "async" only in the caller's
    // sense - it doesn't block the current native callback, but it isn't
    // parallel.
    MainThreadDeferred,
...
// (delivery back to the wire) run where the transport lives.
...
// Used by the bridges' typed call overloads.
