...
SamplerState samplerConfig3 : register(s3);
...
{
    float t0 = (((texture0.Sample(samplerConfig3, input.v0)).x - (uniforms.u5).x) * (uniforms.u5).y);
    float4 t1 = texture1.Sample(samplerConfig3, input.v0);
